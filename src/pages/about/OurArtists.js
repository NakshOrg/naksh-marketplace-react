...
export default function OurArtists() {
  ...
  const getAllArtforms = () => {
    _getAllArtforms().then(({ data: { artforms } }) => {
  ...
  return (
    <Container fluid className={classes.container}>
      ...
      <div className={classes.sectionSubHeading}>
        Naksh has artists from different mediums, styles, age-groups and
        backgrounds - everybody is welcome.
      </div>
      <div style={{ ...globalStyles.flexRowSpace, marginTop: 100 }}>
        <div style={{ margin: 0 }} className={classes.sectionTitle}>
          Our artists
        </div>
        <div
          className={classes.desktopFilter}
          style={{
            display: "flex",
              ...
              return (
                <div
                  ...
                  className={`${classes.pill} ${
                    selectedArtform === item.name ? classes.pillActive : ""
                  }`}
                >
                  {item.name}
                </div>
            ...
            {stateOptions.map((item) => {
              return (
                <div
                  ...
                  className={`${classes.pill} ${
                    selectedLocation === item.value ? classes.pillActive : ""
                  }`}
                >
                  {item.label}
                </div>
