...
const CollectionDetails = () => {
    ...
    const calculateVolume = async () => {
        ...
        const sorted = soldNfts.sort((a, b) => Number(b.timestamp) - Number(a.timestamp))
							...
							<div className="w-full border-b pb-2 flex justify-around items-center">
								<div className="w-1/2 space-y-2  flex flex-col justify-center items-start">
									<h1 className="text-lg">
										<span className="font-bold">{soldNft.nft.title}</span> bought by <span className="font-bold">{soldNft.buyer.substring(0, 7)}...</span> from <span className="font-bold">{soldNft.seller.substring(0, 7)}...</span>
									</h1>
    ...
    return (
        <div className="w-full space-y-10 min-h-screen md:mt-[105px] sm:mt-0">
            ...
            {collection && 
                <>
                    {collection.cover.isGradient && <div className={`w-full h-96 bg-gradient-to-r from-${JSON.parse(collection.cover.uri).fromColor} to-[${JSON.parse(collection.cover.uri).toColor}]`}></div>}
                    {!collection.cover.isGradient && <img src={collection.cover.uri} className={`w-full h-96 flex justify-center items-center`} />}
                        <div className="w-full h-full flex flex-col md:flex-row justify-start items-center md:items-start">
                            <div className="-mt-40 w-11/12 md:w-1/4 h-full md:mx-20 bg-black/75 backdrop-blur-lg rounded-xl p-10 flex flex-col justify-start items-center space-y-5">
                                ...
                                <p className="text-gray-400">Created by {collection.admin.substring(0, 7)}...</p>
                                <p className="">
                                    {!viewMore && collection.about && collection.about.substring(0, 150)}
                                    {viewMore && collection.about}
                                    <span className="text-blue-700 cursor-pointer" onClick={() => setViewMore(!viewMore)}>{' '}view more...</span>
                                </p>
                                ...
                                {collection.admin.toLowerCase() === evmWalletData.address.toLowerCase() && 
                        ...
                        <div className="w-3/4 h-full space-y-5">
                            <div className="w-full space-y-4 md:space-y-0 flex flex-col md:flex-row justify-between items-center">
                                <div className="w-full md:w-1/2 flex justify-start items-center space-x-4">
                                    <h3 className={(isItems ? "border-b-2 border-b-white border-":"") + " cursor-pointer"} onClick={() => setIsItems(true)}>Items</h3>
                                    <h3 className={(!isItems ? "border-b-2 border-b-white border-":"") + " cursor-pointer"} onClick={() => setIsItems(false)}>Activity</h3>
                                </div>
                                <div className="w-full md:w-1/2 flex justify-start md:justify-end items-center space-x-4 md:px-10">
                                    <div className="space-y-2">
                                        <h3 className="text-xl font-bold">{nfts && <>{nfts.length}</>}</h3>
                                        <h3>Items</h3>
                                    </div>
                                    <div className="space-y-2">
                                    ...
                                    {nfts.length > 0 && nfts.map(nft => (
                                        <>
                                            ...
                                            >
                                                <NftCard
                                                    onClick={() =>
                                                        history.push(`/polygon/nftdetails/${nft.nftAddress}/${nft.tokenId.toString()}`)
                                                    }
                                                    image={
                                                        nft.tokenUri.startsWith('ipfs') ? `https://${nft.tokenUri.substring(7)}.ipfs.nftstorage.link` : nft.tokenUri
                                                    }
                                                    title={nft.title}
                                                    nearFee={0}
