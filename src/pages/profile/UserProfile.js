...
export default function UserProfile(props) {
    ...
    const accountId = location?.state?.ownerAccountId ? location?.state?.ownerAccountId : (walletInfo && walletInfo.getAccountId());
    ...
    useEffect(() => {
		if(evmProvider) {
			getUserCollectionsAndAssets(evmWalletData.address).then(res => setUserCollections(res)).catch(e => console.error(e))
            getMintedNFTs(evmWalletData.address).then(res => setOwnedNfts(res)).catch(e => console.error(e))
		}
	}, [evmProvider])
    ...
    const harmonyProfileInfo = () => {
        ...
        return <div className={classes.profileContainer}>
            <img 
                style={{height:100, width:100, borderRadius:100, objectFit:"cover", marginTop:30, alignItems: 'center', justifyContent: 'center'}}
                src={artist?.image ?? profileSvg}
                alt='profile'
            />
            <div style={{fontFamily:"Athelas-bold", fontSize:24, marginTop:10}}>{evmWalletData.address.substring(0, 5)}...</div>
            <div style={{opacity:0.7, fontSize:13, color:"#fff", letterSpacing:1, marginTop:4, wordBreak: "break-word", padding: "0 30px"}}>
    ...
    const renderEVMNfts = () => {
		return allEVMNfts && allEVMNfts.map((nft) => {
			var number;
			var id;
			try {
    ...
    if (noUserFound && !evmWalletData) return <div style={{textAlign:"center", fontSize:24, marginTop:160}}>
        No user found!
    </div>
    ...
    return (
        <div>
            ...
            {evmWalletData && 
                ...
                <div className="w-full h-96 bg-gradient-to-r from-blue-500 to-blue-900"></div>
                    <div className="w-full h-full flex flex-col md:flex-row justify-start items-start">
                        <div className="-mt-40 w-full md:w-1/4 h-full mx-20 bg-black/75 backdrop-blur-lg rounded-xl p-10 flex flex-col justify-start items-center space-y-5">
                            <img src={profileSvg} className="rounded-full bg-white w-40 h-40" />
                            <h1 className="font-bold text-3xl">{evmWalletData.address.substring(0, 6)}...</h1>
                            <p className="text-gray-400">{evmWalletData.address}</p>
                        ...
                        <div className="w-3/4 h-full space-y-5">
                            <div className="w-full flex justify-center items-center space-x-4">
                                <h3 className={(activeTab === 'minted' ? "border-b-2 border-b-white border-":"") + " cursor-pointer text-lg font-bold"} onClick={() => setActiveTab("minted")}>Minted</h3>
                                <h3 className={(activeTab === 'collections' ? "border-b-2 border-b-white border-":"") + " cursor-pointer text-lg font-bold"} onClick={() => setActiveTab("collections")}>Collections</h3>
                                <h3 className={(activeTab === 'owned' ? "border-b-2 border-b-white border-":"") + " cursor-pointer text-lg font-bold"} onClick={() => setActiveTab("owned")}>Owned</h3>
                                <h3 className={(activeTab === 'saved' ? "border-b-2 border-b-white border-":"") + " cursor-pointer text-lg font-bold"} onClick={() => setActiveTab("saved")}>Saved</h3>
                            </div>
                            {activeTab === 'minted'&&
                                <>
                                    {ownedNfts.length <= 0 ? 
                                        ...
                                        <Row>
                                            {ownedNfts.map((nft) => (
                                                ...
                                                >
                                                    <NftCard
                                                        onClick={() => history.push(`/polygon/nftdetails/${nft.nftAddress}/${nft.tokenId}`)}
                            ...
                            {activeTab == 'collections' && 
                                <Row>
                                    {userCollections.map((collection) => (
                                        ...
                                        >
                                            <NftCard
                                                onClick={() => history.push(`/collection/${collection.nftAddress}`)}
