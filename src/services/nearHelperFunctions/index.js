...
export default function NearHelperFunctions(wallet) {
  this.getAllListedNfts = async (allNfts, getAllNft) => {
    const res = await wallet
      .account()
      .viewFunction(configs.nakshMarketWallet, "get_sales_by_nft_contract_id", {
    ...
    const {
      data: { artists },
    } = await _getAllArtists({ sortBy: "createdAt", sort: -1 });
    const filteredNfts = [];
    ...
    allNfts.map((nftItem) => {
      ...
      const artist = artists.find(
        (a) => a._id === nftItem?.metadata?.extra?.artistId
      );
      ...
      if (listedItem) {
        nftItem["listed"] = true;
        nftItem["price"] = utils.format.formatNearAmount(
          listedItem.sale_conditions
        );
  ...
  this.getAllNfts = async (getAllNft) => {
    const res = await wallet
      .account()
      .viewFunction(configs.nakshContractWallet, "nft_tokens", {
        from_index: "0",
        limit: 1000,
      });
  ...
  this.getNftDetails = async () => {
    ...
    allNfts.map((nftItem) => {
      nftItem.metadata["extra"] = JSON.parse(nftItem.metadata.extra);
      const listedItem = listedNfts.find(
        (t) => t.token_id === nftItem.token_id
      );
      const artist = artists.find(
        (a) => a._id === nftItem?.metadata?.extra?.artistId
      );
  ...
  this.getOwnedNfts = async (accountId) => {
    ...
    const res = await wallet
      .account()
      .viewFunction(configs.nakshContractWallet, "nft_tokens_for_owner", {
